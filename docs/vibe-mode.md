...
On a fork build with the attach substrate, each spawned worker also opens a live interactive pane in the herdr terminal: the production `omp attach <workerId> --session-file <parentSessionFile>` client rendered as a **fullscreen alternate-screen TUI** — the pane borrows the alternate screen buffer on first paint, so the shell launch command disappears behind it and the terminal is restored cleanly on exit.

- **Header/status.** A bold `attach <workerId>` header sits above the transcript; the composer's top border shows the worker state (`starting` / `running` / `idle` / `parked` / `revived` / `finished` / `error`, or the connection phase while connecting), plus the current summary, `tool:`, `queued:` count, `working…`, and `last:` result.
- **Transcript.** Bounded and sanitized: 500 rows / 200 chars per line by default, ANSI/control sequences stripped, empty rows suppressed, tail-overlap rows deduped, and a `[trimmed: earlier output dropped]` marker when the bounded window first drops older output. Scrollable; sticks to the bottom unless scrolled up.
- **Composer.** A focused editor composer is the sole focus target. Enter submits, Shift/Ctrl+Enter inserts a newline (multiline drafts), Escape clears the draft, and Ctrl-C clears the draft or — on an empty draft — aborts the in-flight follow-up, sends `bye`, and exits 0.
- **Follow-ups.** Rapidly typed prompts are queued client-side while the worker is unknown, the connection is reconnecting, or another follow-up is in flight, then flushed in order into the worker's turn queue — the same queue `vibe_send` uses. Follow-ups and aborts never kill the worker.
...
- **Requirements.** The parent must run the fork binary (the attach server is vibe-runtime-scoped and boots at worker spawn), herdr must be running with `HERDR_ENV=1`, and the herdr-omp-attach-panes plugin (linked or drop-in) must be installed. The pane env carries only `ATTACH_SOCKET_PATH` and `ATTACH_TOKEN_FILE_PATH` (paths, never the capability; the client reads the 0600 token file itself). `HERDR_OMP_ATTACH_BIN` overrides the client command; `HERDR_OMP_VIBE_PANES=0` opts out.

This is a fork feature: stock omp has no attach substrate, so its workers are steerable only through the director's `vibe_*` tools in the main TUI. The pane is styled as an OMO Slim-style fullscreen overlay, but it is driven by omp's own production attach substrate — the local attach server and the `omp attach` client over the JSON-line socket with a 0600 capability token — not by OMO Slim's architecture. The interactive contract is equivalent to that style: a live bounded transcript with a focused composer (Enter send, multiline, Escape clear, Ctrl-C clear/abort+exit) whose submissions land in the worker's turn queue.
