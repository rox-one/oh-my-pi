...
- `description` is required for:
  - native `.omr` provider skill discovery (`requireDescription: true`)
...
1. `native` (priority 100) — `.omr` user/project skills via `src/discovery/builtin.ts`
2. `omp-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:`, `--extension`/`-e`, or installed plugins under `~/.omr/plugins/node_modules`
...
7. `omp-managed` (priority 5) — auto-learn skills under `~/.omr/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
