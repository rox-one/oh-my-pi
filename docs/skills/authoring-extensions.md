...
That is a working extension. Drop it into `~/.omr/agent/extensions/hello.ts` and restart omp to see the notification.
...
1. Native `.omr` locations discovered through the capability system:
   - `<cwd>/.omr/extensions/`
   - `~/.omr/agent/extensions/`
   - legacy extension paths listed in `.omp/settings.json#extensions` or `~/.omr/agent/settings.json#extensions`
2. Enabled installed plugins under `~/.omr/plugins/node_modules` or a project plugin root — including npm, marketplace, and `omp plugin link` installs — via their `omp.extensions`/`pi.extensions` manifests.
...
The user directory is the active profile's agent directory: the default is `~/.omr/agent`, while `omp --profile <name>` uses `~/.omr/profiles/<name>/agent` (and `PI_CODING_AGENT_DIR` overrides it).
...
omp writes structured logs under the active state root's `logs/` directory (by default `~/.omr/logs/`; debug level is always on, and nothing is written to the console because that would corrupt the TUI). Each filename includes the process ID. Tail today's default-profile logs to see extension load diagnostics:

```
tail -f ~/.omr/logs/omp.$(date +%F).*.log
```
