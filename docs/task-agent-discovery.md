...
OMP discovers user agents from `~/.omr/agent/agents/*.md` and project agents from `.omp/agents/*.md`.
...
Set the role mapping in `~/.omr/agent/config.yml`:
...
- Project `.omr` overrides user `.omr`.
