...
Loads `.omr` rules from:

- project rules: `<cwd>/.omr/rules/*.{md,mdc}` when the cwd's `.omp/` directory is non-empty
...
The active native agent directory is `~/.omr/agent` by default, follows named profiles, and honors `PI_CODING_AGENT_DIR`.
...
- `native` appends project `.omp/rules`, user `~/.omr/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
