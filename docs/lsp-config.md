...
|            | Cwd config dirs: `<cwd>/.omr/lsp.*`, `<cwd>/.claude/lsp.*`, `<cwd>/.codex/lsp.*`, `<cwd>/.gemini/lsp.*`      |
...
The native user config directory follows `PI_CONFIG_DIR` and active profiles; `~/.omr/agent/lsp.json` is the default-profile spelling. This shared config lookup does not use `PI_CODING_AGENT_DIR` as an arbitrary replacement base. Project and cwd sources do not walk ancestors.

**Recommended locations:**

- User-wide preferences → active native agent directory's `lsp.json`
- Project-specific overrides → `<cwd>/.omr/lsp.json`
...
Place the override in `<project>/.omr/lsp.json`:
...
The user-level config in `~/.omr/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
