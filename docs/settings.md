...
| `providers.approvalSimilarityModel` | enum    | `online`  | Classifier for the `Approve Similar <tool> Commands for Session` approval option: `online` (the `TINY` role from `/models`, else `smol`) or a local model (same values as `providers.tinyModel`). Fails safe to a normal approval prompt. See [Tool approval mode](./approval-mode.md).                                                                                                                                                 |
