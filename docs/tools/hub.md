...
- **Jobs** — `wait` (bare or with `ids`), `cancel`, `jobs`. Owner-scoped visibility, watch/unwatch delivery suppression, `acknowledgeDeliveries` on returned completions (all three ops, so observing a settled job stays its delivery), 500 ms `onUpdate` snapshots while waiting, and the `async.pollWaitDuration` fixed/smart wait window. `wait` and `cancel` name specific jobs and print each settled body; `jobs` is a status poll over every owned job and prints rows only — each settled row carries the withheld body's byte count plus where to read it (`agent://<id>` for a task, `wait` on the id otherwise). `jobs` also lists the roster of running subagents with no running job entry.
