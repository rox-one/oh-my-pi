...
User remaps live in `~/.omr/agent/keybindings.yml`. The file is a YAML mapping whose keys are keybinding action IDs and whose values are either one chord string or an array of chord strings. It is not read from `~/.omr/agent/config.yml`, and there is no nested `keybindings` object.
