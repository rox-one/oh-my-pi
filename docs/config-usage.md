...
- OMP native: `~/<PI_CONFIG_DIR>/agent` (normally `~/.omr/agent`; a named profile changes this as described below)
- `~/.claude`
- `~/.codex`
- `~/.gemini`
...
`CONFIG_DIR_NAME` is `.omr` (`packages/utils/src/dirs.ts`). `PI_CONFIG_DIR` changes the OMP user root used by the generic helpers. `PI_CODING_AGENT_DIR` is different: for the default profile it changes `getAgentDir()` consumers such as native discovery, settings, and runtime state, but it does **not** change the generic `getConfigDirs()` / `findConfigFile()` OMP base. Named profiles ignore `PI_CODING_AGENT_DIR`.

## Profiles

A named profile (`omp --profile <name>`, `OMR_PROFILE`, or the legacy fallback `PI_PROFILE`) relocates the OMP user base. `OMR_PROFILE` wins when it is defined, including when it is explicitly empty; `default`, empty, or whitespace selects the default profile. When a profile is active, every OMP-native user-level path written here as `~/.omr/agent/...` normally resolves to `~/.omr/profiles/<name>/agent/...`. `--alias <command>` does not select a profile by itself: paired with `--profile`, it creates a shell shortcut for that profile.
...
Keybindings are the one exception: a named profile merges the default profile's `~/.omr/agent/keybindings.*` under its own `~/.omr/profiles/<name>/agent/keybindings.*`, with the profile file overriding per binding ([#4867](https://github.com/can1357/oh-my-pi/issues/4867)). Keybindings describe the terminal/keyboard in front of the user, which doesn't change with the active profile, so user-level remaps keep working in every profile unless the profile explicitly overrides them. The inherited file is read-only for the profile process — legacy-format migration of the default profile's file only happens when the default profile itself runs.

On macOS and Linux, an existing `$XDG_DATA_HOME/omr`, `$XDG_STATE_HOME/omr`, or `$XDG_CACHE_HOME/omr` can relocate the corresponding data, state, or cache paths. For a named profile, OMP uses an XDG category only when that category already contains `omp/profiles/<name>`; otherwise that category remains under `~/.omr/profiles/<name>`. Run `omp config init-xdg` before relying on XDG paths.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.omp`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.omr/agent` as shorthand for the active profile's agent directory unless an environment override or XDG path is being discussed.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.omr`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: the first present file among `~/.omr/agent/config.yml` and `config.yaml`
...
1. Migrate from `~/.omr/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.omr` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.omr/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.omr/agent/skills`, without requiring the root `.omr` directory itself to be non-empty.
- `SYSTEM.md`, `RULES.md`, and `.omp/AGENTS.md` read user-level files directly and use the nearest non-empty ancestor `.omr` directory for project files. `RULES.md` becomes an always-apply sticky rule. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract.
...
- Skills: `<ancestor>/.omr/skills/*/SKILL.md` and `~/.omr/agent/skills/*/SKILL.md`
...
For `SYSTEM.md`, `RULES.md`, and `.omp/AGENTS.md`, the native provider walks upward to the nearest non-empty project `.omr` directory.
