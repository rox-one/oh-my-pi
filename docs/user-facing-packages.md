...
- Inputs/storage: configuration comes from `python/robomp/.env` and the mounted `~/.omr/agent/models.container.yml`; GitHub webhook events feed a SQLite-backed queue. The Compose deployment persists the database, per-issue worktrees, session transcripts, and logs in the `robomp_data` volume under `/data`.
...
- Inputs/storage: reads `~/.omr/agent/sessions/`; stores aggregates in `~/.omr/stats.db`.
...
- Setup: run `omp browser-relay install`, load the unpacked extension from
  `~/.omr/browser-relay/extension`, then set `browser.relay` or use `app.relay: true`.
...
- Package: private `@oh-my-pi/collab-web`; production client: <https://share.rox.one/>.
