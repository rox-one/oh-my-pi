...
2. Custom provider and model entries from `~/.omr/agent/models.yml`.
...
Stored credentials live in the auth store at `~/.omr/agent/agent.db` for local auth, or in the configured auth-broker snapshot when running in broker mode. (`PI_CODING_AGENT_DIR` relocates the `~/.omr/agent` base, and the auth store moves with it.)
...
A variable already present in the process environment is never overwritten by a `.env` file. Among the files, a value set in `<cwd>/.env` wins over `~/.omr/agent/.env`, which wins over `~/.omr/.env`, which wins over `~/.env`. So a shell-exported `OPENAI_API_KEY` beats every `.env` file, and a project's `<cwd>/.env` beats your home `~/.env`.
...
```yaml
# ~/.omr/agent/config.yml or <project>/.omr/config.yml
disabledProviders:
  - anthropic
  - openai
  - google
...
Project settings live in `<project>/.omr/config.yml`. Use them when one repository must allow or hide a different provider set than your global default:

```yaml
# <project>/.omr/config.yml
disabledProviders:
  - openai
  - openrouter
```
...
Custom providers live in `~/.omr/agent/models.yml` under `providers:`. A provider ID defined there participates in the same selection, credential resolution, and `disabledProviders` rules as built-in providers.
...
**A provider still appears even though I disabled it.** `disabledProviders` arrays are replaced, not merged: a project `<project>/.omr/config.yml` array fully overrides the global one. Verify the _effective_ list for the directory you are in (path-scoped entries only apply at or under their configured path), and confirm the ID is spelled exactly. Use `omp config get disabledProviders` to inspect the merged value (see [Settings](./settings.md)).
