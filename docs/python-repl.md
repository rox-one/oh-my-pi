...
2. Managed venv at `~/.omr/python-env`
...
- **No Python on PATH** — Install a system Python 3.10+ or place a compatible venv at `~/.omr/python-env`. `omp setup python --check` reports the resolved interpreter.
