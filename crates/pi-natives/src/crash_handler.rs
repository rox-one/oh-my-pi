...
//!   `$XDG_STATE_HOME/omr/logs/` on Linux / macOS when the user has migrated to
...
const DEFAULT_CONFIG_DIR: &str = ".omr";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/omr/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "omr";
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/omr/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/omr` exists on disk, and
...
mod tests {
	...
	fn resolve_logs_dir_defaults_under_dot_omp() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.omr/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		let dir = xdg_state_logs(
			...
			Some(OsStr::new("")),
			Path::new("/tmp/pi-natives-test-home/.omr/agent"),
			|_p| true,
		);
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/pi-natives-test-home/.omr/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.omr/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.omr/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/omp/logs")));
	...
	fn default_agent_dir_uses_dot_omp_by_default() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.omr/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	...
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/pi-natives-test-home/.omr/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/pi-natives-test-home/.omr/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/pi-natives-test-home/.omr/logs/native-alloc-99-1.log")
		);
	}
}
