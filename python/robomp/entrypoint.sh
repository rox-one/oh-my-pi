...
rm -rf /srv/agent-home/.agent /srv/agent-home/.omr/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.omr/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.omr/agent ]; then
    cp -a /srv/agent-home-stage/.omr/agent/. /srv/agent-home/.omr/agent/
fi
...
# omp registers daemon project presence under ~/.omr/run at startup, nesting
...
mkdir -p /srv/agent-home/.omr/run
chgrp -R omp /srv/agent-home/.omr/run
chmod -R g+rwX /srv/agent-home/.omr/run
find /srv/agent-home/.omr/run -type d -exec chmod g+s {} +
chmod 2770 /srv/agent-home/.omr/run
