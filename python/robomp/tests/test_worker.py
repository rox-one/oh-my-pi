...
def test_build_extra_env_stages_agent_home(tmp_path: Path, settings: Settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    omp_agent_dir = stage_home / ".omr" / "agent"
    ...
    assert (agent_home / ".omr" / "agent" / "models.yml").is_file()
    ...
    assert (agent_home / ".omr" / "agent").stat().st_mode & 0o777 == 0o755
    assert (agent_home / ".omr" / "agent" / "models.yml").stat().st_mode & 0o777 == 0o644
