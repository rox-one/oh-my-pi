#!/usr/bin/env bun
/**
 * Token-usage audit over the local omp session corpus (~/.omr/agent/sessions/).
 *
 ...
 * storage (~/.omr/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
const SESSIONS_ROOT = path.join(os.homedir(), ".omr", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".omr", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.omr/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.omr/stats-audit-cache.json)\n` +
