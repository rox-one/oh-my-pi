...
(`~/.omr/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.omr/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.omr/agent/sessions/ and populates ss_* tables
...
Verdicts are cached in `~/.omr/stats-audit-cache.json` (keyed by session id +
