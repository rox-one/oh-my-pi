...
- Added per-session approval options to tool approval prompts: "Approve <tool> Commands for Session" skips further prompts for that tool until the session ends, and "Approve Similar <tool> Commands for Session" auto-approves later commands a small classifier judges similar to the approved one, failing safe to a prompt on any classifier error and on a verdict slower than 3 seconds. An unchanged repeat of an approved command is approved without consulting the classifier at all. Neither option is offered for — or can answer — a `deny` policy, pending provider safety checks, or a prompt the tool itself demanded (bash critical patterns and `bash.patterns: prompt` rules). Grants are held in memory only for the session that made them and are released at every conversation boundary — `/new`, `/reset`, fork, rewind/branch, session switch, and process exit ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
...
- Fixed the "Approve Similar" classifier judging commands it could only read part of: approved subjects were cut to 160 characters and the pending call to 400, so a shared harmless beginning could hide the real operation and earn a silent approval. Nothing is cut for the classifier any more — a pending call over 2,000 characters or already shortened by the tool that formatted it, and recorded subjects over 1,000 characters or likewise shortened, are never classified; they produce a normal approval prompt with no model request ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
- Fixed the "Approve Similar" classifier reading a hedged or chatty answer as approval: the verdict was matched by prefix, so "yesterday it worked" and "YES, but destructive" both granted execution, while both classifier prompts ask for exactly one word. Only an answer that is exactly `yes` or `no` counts now — after any leaked `<think>` block, surrounding emphasis or quotes, and trailing punctuation are removed — and anything else produces a normal approval prompt ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
- Fixed the "Approve Similar" classifier taking instructions from the command it was judging: the compared texts were interpolated into its prompt unframed, so a command carrying its own note to the classifier — or one appending a fake "Approved commands" section — could win the `YES` that runs a tool with no prompt (measured: 3 of 3 with the default backend). Each compared text now travels as a single-line JSON string inside a marker generated per request, and the classifier rules read everything inside it as command text ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
- Fixed the "Approve Similar" classifier having no scope rule for the file tools it is offered for: its rubric described shell commands only, so approving a `write` to a project file left an out-of-tree path — `~/.ssh/authorized_keys`, `~/.config/omp/config.yml`, anything reached through `..` — to the model's judgement, and it was measured granting one (1 of 8 with the default backend). The rubric and the docs now name the target as a third property: the approved paths set the scope, and a path outside their project tree is never similar whatever it contains ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
...
- Fixed RPC clients reporting a false idle after an accepted steer or follow-up: `waitForIdle` now reconciles against authoritative session state instead of a stale `agent_end`, bounds that state read by the caller's remaining timeout, and awaits session-owned prompt setup and finalization instead of spinning microtasks.
...
- Unified default, custom, and subagent system prompt assembly into ordered Runtime, System, Append, and Project zones, preserving harness tools, policies, skills, rules, safety constraints, and project context when model behavior is replaced.
