...
- Fixed unknown Anthropic-Messages gateways (e.g. Ramp/Grok) inheriting Groq's 5-image send floor. Image budgets now follow the model's wire API unless the provider id has a measured cap, so those sessions keep the extra frames instead of dropping them on the way out.
