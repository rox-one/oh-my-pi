...
- Fixed snapcompact archiving more frames than the active provider will send. Frame caps now include the provider image budget, so unknown/custom gateways (safe floor 5) no longer render frames that `clampProviderContextImages` then silently drops.
- Fixed unknown gateways (e.g. Ramp/Grok) inheriting Groq's 5-image send floor. Image budgets now follow the model family unless the provider id has a measured cap, so Grok on Ramp keeps extra frames while an unknown model on the same gateway stays at 5.
...
- Added the `providers.approvalSimilarityModel` setting selecting the similarity-classifier backend: `online` (default) or a local on-device model ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
- Added `metadataForProvider(provider)` to the custom-tool context ([#4608](https://github.com/can1357/oh-my-pi/issues/4608)).
...
- Added durable OSC 8 links for task transcripts, task output and patch paths, `/tan` sessions, and background-job artifacts in task cards, Hub rows, and replayed async results ([#3616](https://github.com/can1357/oh-my-pi/pull/3616) by [@wolfiesch](https://github.com/wolfiesch)).
