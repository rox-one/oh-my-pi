...
describe("dedupeContainedContextFiles", () => {
	...
	it("treats files without a depth as less authoritative than project files", () => {
		...
		const files = [file("/project/AGENTS.md", project, 0), file("/home/user/.omr/AGENTS.md", user)];

		expect(paths(dedupeContainedContextFiles(files))).toEqual(["/home/user/.omr/AGENTS.md", "/project/AGENTS.md"]);
	});
