...
function systemdExtension(): Extension {
	return {
		...
		path: "/home/sf/.omr/agent/tools/systemd.ts",
		source: userSource(),
		state: "active",
		raw: {
			name: "systemd",
...
function systemdFactory(sourcePath = "/home/sf/.omr/agent/tools/systemd.ts") {
	return [
		{
...
function toolExtension(): Extension {
	return {
		...
		path: "/home/sf/worlds/personal/.omr/tools/gmail_send.ts",
...
function commandExtension(): Extension {
	return {
		...
		displayName: "triage",
		trigger: "/triage",
		path: "/home/sf/worlds/_template/.omr/commands/triage.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "triage",
...
function skillExtension(): Extension {
	return {
		...
		path: "/home/sf/.omr/agent/skills/hcom/SKILL.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "hcom",
...
function listedSkillExtension(): Extension {
	return {
		...
		path: "/home/sf/.omr/agent/skills/fresh-drive/SKILL.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "fresh-drive",
	...
	test("collapses newlines in factory names and labels to one physical row", () => {
		...
		panel.setToolSource({
			...
			listLiveTools: () => [
				{
					name: "systemd_inspect\ninjected",
					sourcePath: "/home/sf/.omr/agent/tools/systemd.ts",
					label: "inspect\nlabel",
	...
	test("joins a multi-export factory by originating source file", () => {
		...
		const list = new ExtensionList(
			[
				{
					...systemdExtension(),
					path: "/home/sf/worlds/personal/.omr/tools/systemd.ts",
					source: projectSource(),
				},
			],
			{ toolSource: systemdSource("/home/sf/worlds/personal/.omr/tools/systemd.ts") },
		);
		list.setFocused(true);
		const text = Bun.stripANSI(list.render(80).join("\n"));
	...
	test("does not join a shadowed custom-tool row against the winner's live schema", () => {
		...
		panel.setToolSource({
			getLiveTool: () => ({
				...
				description: "Send an email via gog for an authorized personal Gmail account.",
				source: "extension",
				sourcePath: "/home/sf/worlds/personal/.omr/tools/gmail_send.ts",
				parameters: { type: "object", properties: { to: { type: "string" } } },
			}),
		});
		panel.setExtension({
			...
			path: "/home/sf/.omr/agent/tools/gmail_send.ts",
			raw: {
				name: "gmail_send",
				description: "gmail_send custom tool",
				path: "/home/sf/.omr/agent/tools/gmail_send.ts",
		...
		const list = new ExtensionList([
			{
				...
				path: "/home/sf/worlds/personal/.omr/skills/gog-google/SKILL.md",
				source: projectSource(),
			},
		]);
		list.setFocused(true);
 ...
 *
 * Symlink: ~/.omr/agent/tools/cloak.ts → this file.
 */
