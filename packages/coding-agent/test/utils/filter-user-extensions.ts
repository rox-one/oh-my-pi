...
//    or `../`-relative entry pointing somewhere else under `~/.omr/`), plus
//    the legacy non-XDG `~/.omr/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.omr/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/omr/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.omr` when
...
// symlinks `~/.omr/agent/extensions/foo.ts` to a dotfiles repo would see the
