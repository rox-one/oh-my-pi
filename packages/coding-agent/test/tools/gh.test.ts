 ...
 * user's real `~/.omr/wt`. Returns the temp home and a cleanup hook.
 */
interface TempHome {
	home: string;
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	// through the spied homedir instead of `$XDG_DATA_HOME/omr/wt` (CI sets these).
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".omr", "wt", segment));
}
