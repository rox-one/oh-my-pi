...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .omp/ directory", async () => {
		// Layout: tmpDir/.omr/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .omp/ there.
		fs.mkdirSync(path.join(tmpDir, ".omr"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".omr", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .omp/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.omr/   +   tmpDir/sub/.omr/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.omr/, not climb further to tmpDir/.omr/.
		fs.mkdirSync(path.join(tmpDir, ".omr"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".omr"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".omr", "plugins", "installed_plugins.json"));
	});
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.omr/.../installed_plugins.json.
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".omr", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tmpProject, ".omr", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".omr", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".omr", "plugins", "installed_plugins.json");
	});
