 ...
 * depends on:
 *   1. OMP registry lives at path.join(home, ".omr", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".omr")
...
// but we know the value is ".omr" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".omr";
...
let tmpHome: string;
/** ~/.omr/plugins/installed_plugins.json inside tmpHome */
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.omr/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".omr", "plugins", "installed_plugins.json");
