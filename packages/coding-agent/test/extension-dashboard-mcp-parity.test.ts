...
describe("loadAllExtensions MCP parity with /mcp list (issue #3827)", () => {
	...
	beforeEach(async () => {
		...
		await fs.writeFile(
			path.join(projectDir, ".omr", "mcp.json"),
			JSON.stringify({
				mcpServers: {
	...
	test("dashboard re-enable flips enabled:false in mcp.json (PR #3829 review)", async () => {
		...
		const projectMcpPath = path.join(projectDir, ".omr", "mcp.json");
	...
	test("dashboard disable on a config-resident server writes enabled:false (not denylist)", async () => {
		...
		const projectConfig = await readMCPConfigFile(path.join(projectDir, ".omr", "mcp.json"));
	...
	test("dashboard re-enable updates the row's non-primary source mcp.json before denylisting", async () => {
		const alternatePath = path.join(projectDir, ".omr", ".mcp.json");
		await Bun.write(
			alternatePath,
			JSON.stringify({
				mcpServers: {
