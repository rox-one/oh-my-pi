 ...
 *
 * A named profile relocates the agent directory to ~/.omr/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.omr/agent/mcp.json) via `ctx.home`, so a profile never saw its own
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.omr/profiles/<name>/agent).
		...
		await writeMcpJson(path.join(tempHome, ".omr", "agent"), {
	...
	test("default profile loads the user server from ~/.omr/agent", async () => {
