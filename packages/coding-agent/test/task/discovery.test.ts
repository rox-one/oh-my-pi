...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".omr", "plugins");
...
describe("discoverAgents", () => {
	...
	test("loads OMP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".omr", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".omr", "agents", "omp-test-agent.md"), OMP_AGENT_MD);
		...
		expect(projectAgentsDir).toBe(path.join(projectDir, ".omr", "agents"));
	});

	test("loads agents from OMP npm plugins under <home>/.omr/plugins/node_modules", async () => {
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		await fs.mkdir(path.join(projectDir, ".omr"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".omr", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	...
	test("explicit-only CLI roots expose only explicitly named package agents", async () => {
		...
		await fs.mkdir(path.join(projectDir, ".omr"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".omr", "settings.json"), JSON.stringify({ extensions: [settingsExt] }));
		await writeOmpPluginAgent(tempHome);
