 ...
 *
 * Enforces — in this order — the noise filter, session-scoped exact-text dedupe for
 * ordinary notes and identity dedupe for validated policies (FIFO-evicted at
 * {@link DEFAULT_HISTORY_CAPACITY}), and a per-update rate limit of one accepted note per advisor model prompt. Suppressed calls
...
export class AdvisorEmissionGuard {
	...
	accept(note: string, policyKey?: string): boolean {
		...
		const key = policyKey === undefined ? `note\u0000${normalizedNote}` : `policy\u0000${policyKey}`;
