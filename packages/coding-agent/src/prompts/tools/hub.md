...
- **`jobs`**: status snapshot of every job without waiting. Rows only — a settled row consumes auto-delivery but NEVER repeats the result body; it names the size and where to read it (`agent://<id>` for a task, `wait` on the id otherwise). Also names running subagents with no job entry — coordinate with those via `send`.
