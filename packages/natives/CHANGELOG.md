...
- Fixed native addon loading leaving stale `~/.omr/natives/<version>` cache directories behind after updates; successful loads now remove older version directories best-effort.
...
- Updated native module loader to check `XDG_DATA_HOME` environment variable for native addon location before falling back to `~/.omr/natives`
...
- Added automatic extraction of embedded native addon to `~/.omr/natives/<version>` on first run for compiled binaries
...
- Embedded native addon payload for compiled binaries and extract to `~/.omr/natives/<version>` on first run
