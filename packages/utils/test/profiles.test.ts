...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfileEnv = process.env.OMR_PROFILE;
	...
	afterEach(async () => {
		...
		if (originalOmpProfileEnv === undefined) {
			delete process.env.OMR_PROFILE;
		} else {
			process.env.OMR_PROFILE = originalOmpProfileEnv;
		}
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "omr", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "omr", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "omr", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "omr", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "omr", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "omr", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		// Fresh install: XDG vars are set (typical Linux) but no $XDG/omr exists yet.
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "omr"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "omr"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid OMR profile");
		expect(() => setProfile("work/team")).toThrow("Invalid OMR profile");
	});
	...
	it("does not restore a profile-derived agent dir as the default baseline", () => {
		// Reproduces a child process that inherited OMR_PROFILE=work plus the
		...
		// Re-snapshot exactly as module load would, now that OMR_PROFILE and the
...
describe("profile env + name validation", () => {
	it("honors OMR_PROFILE precedence and treats empty/default as the default profile", () => {
		// OMR_PROFILE is canonical and wins over the legacy PI_PROFILE fallback.
		expect(resolveProfileEnv("work", "other")).toBe("work");
		// PI_PROFILE is consulted only when OMR_PROFILE is undefined.
		expect(resolveProfileEnv(undefined, "work")).toBe("work");
		// An explicitly-empty OMR_PROFILE selects the default profile; it must NOT
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid OMR profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid OMR profile");
...
describe("dirs module import behavior", () => {
	...
	it("ignores inherited profile agent dir when OMR_PROFILE explicitly selects default", async () => {
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "omr", "profiles", "work");
