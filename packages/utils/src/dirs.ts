/**
 * Centralized path helpers for omr config directories.
 *
 * Uses PI_CONFIG_DIR (default ".omr") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/omr/. This requires running `omr config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, omr trusts that the migration has been done.
 */
...
/** App name (e.g. "omr") */
export const APP_NAME: string = "omr";

/** Config directory name (e.g. ".omr") */
export const CONFIG_DIR_NAME: string = ".omr";
...
const PROFILE_ENV_KEYS = ["OMR_PROFILE", "PI_PROFILE"] as const;
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid OMR profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the two profile env vars. `OMR_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `OMR_PROFILE` is undefined. An
 * explicitly-empty `OMR_PROFILE` therefore selects the default profile rather
...
function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.OMR_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * OMR_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid OMR profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".omr" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".omr/agent" or PI_CONFIG_DIR + "/agent"). */
 ...
 * variables are set, paths are redirected under $XDG_*_HOME/omr/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/omr/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/omr once `omr config init-xdg` has migrated
		...
		// profile could resolve to `~/.omr/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/omr exists yet) and then silently move to
		// `$XDG_*_HOME/omr/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.omr/agent/sessions → $XDG_DATA_HOME/omr/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `OMR_PROFILE` explicitly selected the
 ...
 * `~/.omr/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.omr). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.OMR_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.omr/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.omr). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.omr/*)
// =============================================================================

/** Get the reports directory (~/.omr/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.omr/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get this process's dated log path (~/.omr/logs/omp.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.omr/plugins or its XDG equivalent).
 *
 ...
 * fall back to `OMP_WORKTREE_DIR` or the `~/.omr/wt` default.
 *
 ...
 * `worktree.base` setting), then the `~/.omr/wt` default. The env var and the
...
/** Get the SSH control socket directory (~/.omr/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.omr/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.omr/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.omr/agent/python-gateway; XDG default: $XDG_STATE_HOME/omr/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.omr/puppeteer). */
export function getPuppeteerDir(): string {
	return dirs.rootSubdir("puppeteer", "cache");
}

/** Get the browser relay extension install directory (~/.omr/browser-relay). */
...
/** Get the auto-QA grievances SQLite database path (~/.omr/autoqa.db; XDG: $XDG_DATA_HOME/omr/autoqa.db). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.omr/wt/`).
...
/** Get the path to a single worktree directory (~/.omr/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.omr/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.omr/cache/github-cache.db).
...
/**
 * Get the encrypted auth-broker snapshot cache path (~/.omr/cache/auth-broker-snapshot.enc).
...
/** Get the last-seen-changelog-version marker file (~/.omr/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.omr/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.omr/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/omr/cache/document-conversions). */
...
/** Get the secret placeholder key path (~/.omr/agent/secret-placeholder.key; XDG default: $XDG_STATE_HOME/omr/secret-placeholder.key). Adopts a legacy key on first XDG resolution. */
...
/** Root directory containing every per-project daemon runtime scope (~/.omr/run/daemons; XDG default: $XDG_STATE_HOME/omr/run/daemons). */
export function getDaemonRuntimeRoot(): string {
	return dirs.rootSubdir(path.join("run", "daemons"), "state");
}

/** Get the daemon runtime directory for a project (~/.omr/run/daemons/<hash>; XDG default: $XDG_STATE_HOME/omr/run/daemons/<hash>). */
...
/** Get the provider in-flight root directory (~/.omr/run/provider-inflight; XDG default: $XDG_STATE_HOME/omr/run/provider-inflight). */
export function getProviderInFlightRoot(): string {
	return dirs.rootSubdir(path.join("run", "provider-inflight"), "state");
}

/** Get the marketplaces registry path (~/.omr/marketplaces.json; XDG default: $XDG_DATA_HOME/omr/marketplaces.json). Adopts a legacy registry on first XDG resolution. */
...
/**
 * Persistent per-install UUID stored at `~/.omr/install-id`.
 *
 ...
 * `~/.omr/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.omr/install-id`) regardless of the
