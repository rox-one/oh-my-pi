...
//     progress status closes the pane (belt and braces with the lifecycle
//     terminal event).
...
//     `<HERDR_OMP_ATTACH_BIN, default "omp"> attach <workerId> --session-file
//     <quoted parentSessionFile>`.
...
//                                     "omp"; passed verbatim, e.g. "bun run
//                                     /path/to/client.ts")
//
...
export class AttachPaneController {
  constructor(options) {
    ...
    this.#attachBin = typeof options.attachBin === "string" && options.attachBin.length > 0 ? options.attachBin : DEFAULT_ATTACH_BIN;
  ...
  handleShutdown() {
    try {
      for (const entry of [...this.#attaches.values()]) {
        if (entry.paneId && !entry.closed) {
          this.#enqueue(async () => {
            try {
              const owned = await this.#verifyOwnership(entry);
              if (!owned) {
...
export default function (pi, processEnv = process.env) {
  ...
  const deps = {
    ...
    attachBin: typeof rawBin === "string" && rawBin.length > 0 ? rawBin : DEFAULT_ATTACH_BIN,
